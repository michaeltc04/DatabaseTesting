 ...
 * Many thanks to markushi for original code
 */
public class CircleButton extends ImageView {

    private static final int PRESSED_COLOR_LIGHTUP = 255 / 25;
    private static final int PRESSED_RING_ALPHA = 65;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        float px = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 40, dm);
        float linePx = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 8, dm);

        Paint whitePaint = new Paint();
        whitePaint.setColor(Color.WHITE);
        whitePaint.setAntiAlias(true);
        whitePaint.setStrokeWidth((float) 6);

        canvas.drawCircle(w - px, centerY, pressedRingRadius + animationProgress, focusPaint);
        canvas.drawCircle(w - px, centerY, outerRadius - pressedRingWidth, circlePaint);

        canvas.drawLine(w - px, centerY, w - px + linePx, centerY, whitePaint);
        canvas.drawLine(w - px, centerY, w - px - linePx, centerY, whitePaint);
        canvas.drawLine(w - px, centerY, w - px, centerY + linePx, whitePaint);
        canvas.drawLine(w - px, centerY, w - px, centerY - linePx, whitePaint);
